...
from active_semi_clustering.semi_supervised.pairwise_constraints import COPKMeans as CKMeans
...
def run_cop_kmeans(x, nmb_clusters, ml_constraint, verbose=False):
    """Runs cop_kmeans.
    Args:
        x: data
        nmb_clusters (int): number of clusters
    ...
    # implementation of copk-means
    clusterer = CKMeans(n_clusters=nmb_clusters)
    ...
    # perform the training
    try:
        clusterer.fit(x, ml=ml_constraint)
    except:
        pass
    ...
    global epoch
    plt.savefig("tsne_plots/tsne_cop_kmeans_deep_"+str(epoch)+".png")
    plt.clf()
    plt.scatter(x_embedded[:,0],x_embedded[:,1],c=[i//300 for i in range(x.shape[0])])
    plt.savefig("tsne_plots/tsne_cop_kmeans_true_"+str(epoch)+".png")
    plt.clf()
    epoch+=1
    return clusterer.labels_, 0
...
class Kmeans(object):
    ...
    def cluster(self, data, verbose=False):
        ...
        xb = preprocess_features(data, pca=40)
...
class COPKMeans(object):
    ...
    def cluster(self, data, verbose=False):
        ...
        ml_constraint = []
        exp_length = 100
        for i in range(int(xb.shape[0]/exp_length)):
            for j in range(exp_length-1):
                ml_constraint.append((i*exp_length+j,i*exp_length+j+1))
        # cluster the data
        I, loss = run_cop_kmeans(xb, self.k, ml_constraint, verbose)
        self.images_lists = [[] for i in range(self.k)]
        for i in range(len(data)):
            self.images_lists[I[i]].append(i)
