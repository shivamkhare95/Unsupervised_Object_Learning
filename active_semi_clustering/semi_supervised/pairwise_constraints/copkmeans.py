...
class COPKMeans:
    ...
    def fit(self, X, y=None, ml=[], cl=[]):
        ...
        cluster_centers = self._init_cluster_centers(X,ml)
        ...
        for iteration in range(self.max_iter):
            ...
            # Estimate means
            cluster_centers = self._get_cluster_centers(X, labels, prev_cluster_centers)
    ...
    def _init_cluster_centers(self, X, ml):
        a = list(range(X.shape[0]))
        for (i,j) in ml:
            a.remove(j)
            
        return X[np.random.choice(a, self.n_clusters, replace=False), :]
...
#             raise EmptyClustersException
    ...
    def _get_cluster_centers(self, X, labels, prev_cluster_centers):
        return np.array([X[labels == i].mean(axis=0) if X[labels==i].size>0 else prev_cluster_centers[i] for i in range(self.n_clusters) ])
